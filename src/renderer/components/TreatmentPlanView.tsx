...
import { createTreatmentPlan, getTreatmentPlans } from "../api/patientsApi";
...
export function TreatmentPlanView({ patient }: TreatmentPlanViewProps) {
  ...
  const loadPlans = async () => {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "No fue posible cargar el plan de tratamiento.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error(requestError);
      setError(requestError instanceof Error ? requestError.message : "No fue posible guardar el plan de tratamiento.");
    } finally {
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-3xl bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-medium text-teal-700">Paciente seleccionado</p>
            <h3 className="mt-1 text-2xl font-semibold text-slate-900">{patient.fullName}</h3>
            <p className="mt-1 text-sm text-slate-500">
              Codigo {patient.userCode} · Documento {patient.personalData.documentType} {patient.personalData.documentNumber}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="rounded-2xl bg-teal-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-teal-950/20 transition hover:bg-teal-700"
            >
              Agregar Plan
            </button>
          </div>
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr className="text-left text-xs uppercase tracking-wide text-slate-500">
                ...
                <th className="px-6 py-4">Diagnostico / Procedimiento</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-10 text-center text-slate-500">
                    Aun no hay planes cargados para este paciente.
                  </td>
                </tr>
              ) : (
                plans.map((plan) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-4">
          <div className="w-full max-w-3xl rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
              <div>
                <p className="text-sm font-medium text-teal-700">Agregar Plan</p>
            ...
            <form className="grid gap-4 px-6 py-6 md:grid-cols-2" onSubmit={handleSubmit}>
              ...
              <Field label="Diagnostico / Procedimiento" className="md:col-span-2">
                <input
                  type="text"
              ...
              <div className="md:col-span-2 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="rounded-2xl bg-teal-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-teal-950/20 transition hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
...
const inputClass =
  "rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm outline-none transition focus:border-teal-500 focus:ring-2 focus:ring-teal-100";
